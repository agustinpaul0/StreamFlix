...
const PopularMedia = () => {
  ...
  const { data: movieData, error: movieError, isLoading: isLoadingMovies } = useQuery<Movie[], Error>({
    queryKey: ["popularMovies"],
    queryFn: () => getPopularMovies(POPULAR_MOVIES_URL),
  });

  const { data: tvData, error: tvError, isLoading: isLoadingTV } = useQuery<Series[], Error>({
